...
# --- Data Loading ---
# Read Portfolio Returns from an Excel file
Ret <- read_xlsx('../Data/Portfolios.xlsx')
# Read Fama-French Factors from an Excel file
FFactors <- read_xlsx('../Data/FF_Factors.xlsx')
# Read Uncertainty data (including EPU) from an Excel file
Uncertainty <- read_xlsx('../Data/Uncertainty.xlsx')
# Read Consumption data from an Excel file
Consumption <- read_xlsx('../Data/Consumption.xlsx')
...
# Combine the factors (Market, Consumption Change, EPU Change) into a single matrix
Factors <- cbind(Mkt, C, EPU)
...
for (i in 1:n2) {
  # Fit linear model: ExRet_i = alpha_i + beta_Mkt*Mkt + beta_dC*dC + beta_EPU*EPU + epsilon_i
  # Use lm() with the combined Factors matrix. lm automatically includes an intercept.
  lm_result <- lm(ExRet[, i] ~ Mkt + C + EPU)
...
FirstStageReg <- data.frame(Mkt = Betas[, 1], dC = Betas[, 2], EPU = Betas[, 3])
...
rownames(SecondStage) <- c('Lambda', 't-stat', 't-stat HAC', 't-stat Shanken')
# Set column names to factor names
colnames(SecondStage) <- c('Mkt', 'C', 'EPU')
