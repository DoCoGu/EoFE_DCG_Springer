...
# Compute Temperature and Consumption Growth
...
for i in 1:n2
    df = DataFrame(Mkt=Factors[:, 1], ExRet=ExRet[:, i], C=Factors[:, 2], T=Factors[:, 3])
    model = lm(@formula(ExRet ~ Mkt + C + T), df)
...
for j in 1:n1
    ...
    rename!(df, ["ExRet", "Mkt", "C", "T"])
    model_j = lm(@formula(ExRet ~ 0 + Mkt + C + T), df)
...
# Write to CSV
CSV.write("FirstStage_T.csv", FirstStageReg)
CSV.write("SecondStage_T.csv", SecondStage)
