...
function newey_west_se(X::Matrix, y::Vector, bandwidth::Int)
    ...
    for l in 0:bandwidth
        # Calculate the Newey-West weight for the current lag (Bartlett kernel)
        w_l = 1.0
        if l > 0
            w_l -= l / (bandwidth + 1)
        end
        ...
        # The loop starts from l + 1 because we need t - l >= 1
        for t in (l + 1):n
            xt = X[t, :]' # Transpose the row vector X[t, :] to get a column vector
            xt_l = X[t - l, :] # Row vector from the lagged observation X[t - l, :]
        ...
        # For lag 0 (l=0), Γ_0 is added. For lags > 0, w_l * (Γ_l + Γ_l') is added.
        S = S .+ w_l * (Γ_l + Γ_l')
        if l == 0
            # At lag 0, the term Γ_0 is added twice in the loop (as Γ_l and Γ_l').
            # We need to remove one instance of Γ_0.
            S = S .- Γ_l
        end
    end
...
# --- Data Import ---
# Load Portfolio Returns from an Excel file (assuming 'PortfoliosLong.xlsx')
Ret = DataFrame(XLSX.readtable("../Data/PortfoliosLong.xlsx", "Portfolios"))
# Load Fama-French Factors from an Excel file (assuming 'FF_FactorsLong.xlsx')
Factors = DataFrame(XLSX.readtable("../Data/FF_FactorsLong.xlsx", "Factors"))

# Load Temperature data from an Excel file
Temperature = DataFrame(XLSX.readtable("../Data/Temperature.xlsx", "Temperature"))
# Extract Temperature data (assuming it's in the 2nd column) and convert to a Float64 Vector
Temperature = convert(Vector{Float64}, Temperature[:, 2])

# Load Consumption data from an Excel file (assuming 'ConsumptionLong.xlsx')
Consumption = DataFrame(XLSX.readtable("../Data/ConsumptionLong.xlsx", "Consumption"))
# Extract Consumption data (assuming it's in the 2nd column) and convert to a Float64 Vector
...
T = diff(log.(Temperature))
# Consumption growth: percentage change (Current Consumption / Previous Consumption) - 1
# Note: Indexing starts from 1 in Julia, so 2:end and 1:end-1 are used for difference/change
...
# Combine the factors (Market, Consumption Change, Temperature Change) into a Matrix
Factors = hcat(Mkt, C, T)
...
for i in 1:n2
    ...
    df_n = DataFrame(Mkt=Factors[:, 1], ExRet=ExRet[:, i], C=Factors[:, 2], T=Factors[:, 3])

    # Fit linear model using GLM: ExRet_i = alpha_i + beta_Mkt*Mkt + beta_dC*dC + beta_T*T + epsilon_i
    # Note: GLM's lm function automatically includes an intercept unless specified otherwise
    model = lm(@formula(ExRet ~ Mkt + C + T), df_n)
...
rename!(df, ["Mkt", "C", "T", "MeanRet"]) # Rename columns to factor names and dependent variable
...
# Using `0 +` in the formula suppresses the intercept (assuming lambda_0 = 0 in the cross-sectional model)
model2 = lm(@formula(MeanRet ~ 0 + Mkt + C + T), df)

# Extract factor risk premia (Lambdas) and their standard errors
Lambda = coef(model2) # Factor risk premia (coefficients from the second stage)
SE = stderror(model2) # Standard errors of Lambdas (standard OLS standard errors)
...
# Formula: (BtB_inv * Betas' * VarCovErr * Betas * BtB_inv * (1 + Lambda' * inv(Sigma_f) * Lambda) + Sigma_f) / n1
...
for j in 1:n1
    ...
    rename!(df_t, ["ExRet", "Mkt", "C", "T"]) # Rename columns
    ...
    model_j = lm(@formula(ExRet ~ 0 + Mkt + C + T), df_t)
...
for k in 1:nF
    ...
    bandwidth = 1 # Bandwidth for Newey-West (adjust as needed, original code used 1)
    ...
    # Extract the Newey-West standard error for the intercept (which is the standard error of the mean of LambdaFull[:, k])
    SE_NW[k] = SE_t[1]
end
...
rename!(FirstStageReg, [:Mkt, :dC, :T]) # Rename columns to factor names
...
SecondStage = DataFrame(
    ...
    rowindex = ["Mkt", "dC", "T"] # Row names for the statistics (factor names)
)
# Stack the DataFrame to long format (combines Lambda, tstat, etc. into a single column 'value')
...
XLSX.writetable("FirstStage_T.xlsx", FirstStageReg; overwrite=true)
# Write the Second Stage results table to a XLSX file
XLSX.writetable("SecondStage_T.xlsx", SecondStage; overwrite=true)
